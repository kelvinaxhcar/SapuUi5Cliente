sap.ui.define([
	...
	return Controller.extend("invent.clientes.controller.Inserir", {
		...
		formatarDadosCliente: function (ModelCliente) {
			let cliente = ModelCliente;
			let cpf = cliente.cpf.replace(/[^0-9]/g, '');
			cliente.cpf = cpf;
			let telefone = cliente.telefone.replace(/[^0-9]/g, '');
		...
		salvarNoBancoDeDados: function () {
			...
			if (cliente != null) {
				this.formatarDadosCliente(this.getClienteModel());
		...
		teste: function () {
			console.table(this.getClienteModel());
			this.getClienteModel()
